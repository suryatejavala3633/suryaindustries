...
const OperationsDashboard: React.FC = () => {
  ...
  const ackStats = useMemo(() => {
    // Calculate total ACKs that should be produced from available paddy
    const totalPaddyQuintals = paddyStats.totalQuintals;
    const averageOutturnRate = 0.675; // Average of 68% (boiled) and 67% (raw)
    const ricePerAck = 287.1; // Quintals per ACK
    
    // Total possible ACKs from available paddy
    const totalPossibleACKs = Math.floor((totalPaddyQuintals * averageOutturnRate) / ricePerAck);
  ...
  const stockStatus = useMemo(() => {
    ...
    return {
      ...
      frk: { total: totalFRK, used: usedFRK, remaining: remainingFRK },
      stickers: { total: totalStickers, used: usedStickers, remaining: remainingStickers }
    };
  }, [gunnyStocks, frkStocks, rexinStickers]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg p-4 border border-blue-200">
                <h3 className="text-lg font-semibold text-blue-900 mb-4">ACK Production Status</h3>
                <ProgressBar 
                ...
                <div className="text-sm text-blue-700">
                  <div className="flex justify-between">
                    <span>Possible ACKs:</span>
                    <span className="font-semibold">{formatNumber(ackStats.totalPossibleACKs)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Produced:</span>
                    <span className="font-semibold">{formatNumber(ackStats.acksProduced)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Remaining:</span>
              ...
              {/* FCI Delivery Progress */}
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                <h3 className="text-lg font-semibold text-green-900 mb-4">FCI Delivery Status</h3>
                <ProgressBar 
                  current={ackStats.acksDelivered} 
                  total={ackStats.acksProduced} 
                  label="ACKs Delivered" 
                />
                <div className="text-sm text-green-700">
                  <div className="flex justify-between">
                    <span>Delivered:</span>
                    <span className="font-semibold">{formatNumber(ackStats.acksDelivered)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>In Transit:</span>
                    <span className="font-semibold">{formatNumber(ackStats.acksInTransit)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Pending:</span>
                    <span className="font-semibold">{formatNumber(ackStats.acksPending)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200">
                <h3 className="text-lg font-semibold text-purple-900 mb-4">Overall Progress</h3>
                <ProgressBar 
                  current={ackStats.acksDelivered} 
                  total={ackStats.totalPossibleACKs} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatsCard
            title="ACKs Produced"
            value={`${formatNumber(ackStats.acksProduced)}/${formatNumber(ackStats.totalPossibleACKs)}`}
            subtitle={`${getProgressPercentage(ackStats.acksProduced, ackStats.totalPossibleACKs)}% of possible ACKs`}
            icon={<Factory className="h-6 w-6" />}
            color="from-emerald-500 to-emerald-600"
          />
          <StatsCard
            title="ACKs Delivered"
            value={`${formatNumber(ackStats.acksDelivered)}/${formatNumber(ackStats.acksProduced)}`}
            subtitle={`${formatNumber(ackStats.acksInTransit)} in transit, ${formatNumber(ackStats.acksPending)} pending`}
            icon={<Truck className="h-6 w-6" />}
            color="from-green-500 to-green-600"
          />
          <StatsCard
            title="Overall Progress"
            value={`${getProgressPercentage(ackStats.acksDelivered, ackStats.totalPossibleACKs)}%`}
            subtitle="Complete delivery progress"
